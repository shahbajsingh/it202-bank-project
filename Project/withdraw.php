...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if ($account['balance'] < $amount) {
            throw new Exception("Insufficient funds for this withdrawal");
        }
        ...
        $stmt = $db->prepare("INSERT INTO Transactions (account_src, account_dest, balance_change, transaction_type, memo, expected_total) 
                            VALUES (:account_id, :world_account_id, :balance_change, 'withdraw', :memo, :account_balance)");
        $stmt->execute([
        ...
        $stmt = $db->prepare("INSERT INTO Transactions (account_src, account_dest, balance_change, transaction_type, memo, expected_total) 
                            VALUES (:world_account_id, :account_id, :balance_change, 'withdraw', :memo, :world_balance)");
        $stmt->execute([
            ':world_account_id' => $world_account_id,
            ':account_id' => $account_id,
            ':balance_change' => $amount,
        ...
        $db->commit();
        flash_message("Withdrawal successful!", 'success');
